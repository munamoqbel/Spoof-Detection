...
| 12 | Coast = Phi_acc * state is the error-state time update, not an INS mechanization; meaning of kfState / reset_state depends on the host's feedback convention | blocker | **open — needs host code**: with a persistent (non-zeroed) error state propagated at 100 Hz by the host, Phi_acc * delta_x is the correct INS-only error-state coast; what `reset_state` must do to the mechanization at latch/commit depends on how the host applies and resets its feedback vector |
| 13 | kfState mirror + Eq. 49 collapse if the host zeroes its error state after feedback | blocker | **open — needs host code** (user reports the state is NOT zeroed; an accumulated-separation form `d <- Phi d + (kfState - priorState)` is ready as the fallback if it is) |
| 14 | reset_state / reseed / feedback semantics only defined for the linear total-state harness | major | **open — needs host code** (same as 12) |
