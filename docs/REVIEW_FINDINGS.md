...
| 14 | reset_state / reseed / feedback semantics only defined for the linear total-state harness | major | **fixed**: `nav.applyCorrection/correction/covar` at latch and commit, `kfCommand.reseedKF` = copy the coasting KF; harness (`recovery_nav_sim`, `scheduler_sim`) follows the same contract |
