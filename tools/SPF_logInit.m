...
spfLog.resetInhibit = false(numEpochs, 1);       % host protective reset held off by the gate
spfLog.budgetOut   = false(numEpochs, 1);        % coast budget exceeded
spfLog.nis         = zeros(numEpochs, 1);        % host KF consistency y' S^-1 y / numMeas (optional)
spfLog.innMean     = zeros(numEpochs, 1);        % mean of the live innovations [m]: common-mode part (receiver clock)
spfLog.innStd      = zeros(numEpochs, 1);        % spread of the live innovations [m]: geometry part (position)
